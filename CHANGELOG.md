...
- Add karpenter support. For a node pool, set `global.nodePools.<name>.type` to `karpenter` to have karpenter manage a pool instead of an EKS managed node group. The karpenter bundle is installed regardless of whether karpenter node pools are configured, so that removing the last karpenter pool leaves karpenter running long enough to clean up its nodes (this is consistent with [cluster-aws](https://github.com/giantswarm/cluster-aws)). Private subnets get tagged with `karpenter.sh/discovery` to tell them apart from secondary Cilium pod subnets, which CAPA labels `role: private` as well. For now, karpenter can only run on static node pool, not on karpenter-provisioned node pools, to ensure it keeps running and doesn't get stopped while doing any provisioning.
...
[2.1.1]: https://github.com/giantswarm/cluster-eks/compare/v2.1.0...v2.1.1
[2.1.0]: https://github.com/giantswarm/cluster-eks/compare/v2.0.0...v2.1.0
[2.0.0]: https://github.com/giantswarm/cluster-eks/compare/v1.3.0...v2.0.0
